...
from data_loader import funding_data, unit_data, units_map
...
def get_image_story(impath, imw=83, imh=65, title=None, style=None, tech_pub_count=0):
    ...
    if title == "Publications by category":
        ...
        out_story.append(Paragraph("<font color='#95C11E' name=Arial-bold>No. of Tech Dev Publications 2021 - 2023:</font> {}".format(
                            tech_pub_count), styles[text_style]))
    return out_story
...
styles.add(ParagraphStyle(name="inner_heading", parent=styles["Heading1"], fontName="Arial",
                          fontSize=12, color="#FF00AA", leading=11, afterspace=0))

styles.add(ParagraphStyle(name="page_text", parent=styles["Normal"], fontName="Arial",
                          fontSize=10, bold=0, color="#000000", leading=13))

styles.add(ParagraphStyle(name="page_text_s", parent=styles["Normal"], fontName="Arial",
                          fontSize=9.5, bold=0, color="#000000", leading=13))

styles.add(ParagraphStyle(name="bar_plot_title", parent=styles["Heading1"], fontName="Arial",
                          fontSize=12, color="#FF00AA", leftIndent=18, leading=0, afterspace=0))

styles.add(ParagraphStyle(name="pie_plot_title", parent=styles["Heading1"], fontName="Arial",
                          fontSize=12, color="#FF00AA", leading=5, afterspace=0, alignment=TA_CENTER))
...
for unit in units_map.keys():
    ...
    if cat_plot:
        caller_dict['ctbar'] = dict(impath=cat_plot, title="Publications by category", style=styles["bar_plot_title"],tech_pub_count=tech_pub_count, **unit_grph.get("figsize", {}).get("cat", {}))
    else:
        print("{} - missing category bar".format(unit))
    if jif_plot:
        caller_dict['jfbar'] = dict(impath=jif_plot, title="Publications by JIF", style=styles["bar_plot_title"], **unit_grph.get("figsize", {}).get("jif", {}))
    else:
    ...
    if usr_y1_plot:
        caller_dict['usr_y1'] = dict(impath=usr_y1_plot, title="Users 2021", style=styles["pie_plot_title"], **unit_grph.get("figsize", {}).get("u_y1", {}))
    else:
    ...
    if usr_y2_plot:
        caller_dict['usr_y2'] = dict(impath=usr_y2_plot, title="Users 2022", style=styles["pie_plot_title"], **unit_grph.get("figsize", {}).get("u_y2", {}))
    else:
    ...
    if usr_y3_plot:
        caller_dict['usr_y3'] = dict(impath=usr_y3_plot, title="Users 2023", style=styles["pie_plot_title"], **unit_grph.get("figsize", {}).get("u_y3", {}))
    else:
    ...
    story.append(Paragraph("<font name=Arial-bold>PSD:</font> {}".format(unit_stat['PSD']), styles[text_style]))
    story.append(Paragraph("<font name=Arial-bold>HU:</font> {}".format(unit_stat['HOU']), styles[text_style]))
    story.append(Paragraph("<font name=Arial-bold>Host University:</font> {}".format(unit_stat['H_uni']), styles[text_style]))
    story.append(Paragraph("<font name=Arial-bold>SciLifeLab unit since:</font> {}".format(unit_stat['SLL_since']), styles[text_style]))
    story.append(Paragraph("<font name=Arial-bold>FTEs:</font> {}".format(unit_stat['FTEs']), styles[text_style]))
    ...
    story.append(Paragraph("<font color='#95C11E' name=Arial-bold>Funding 2023 (kSEK)</font>", styles[head_style]))
    story.append(Paragraph("<font name=Arial-bold>SciLifeLab:</font> {}".format(str(unit_stat['Fund_SLL'])), styles[text_style]))
    story.append(Paragraph("<font name=Arial-bold>Other:</font> {}".format(str(unit_stat['Fund_other'])), styles[text_style]))
    story.append(Paragraph("<font name=Arial-bold>User Fees:</font> {}".format(str(unit_stat['Fee_total'])), styles[text_style]))
