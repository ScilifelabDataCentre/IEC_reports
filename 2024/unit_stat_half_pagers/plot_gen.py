...
def publication_plot(unit, pformat="png"):
    ...
    if any(pd.concat([tecdev, collab, service, nocat]).Count):
        ...
        fig.update_layout(
            ...
            autosize=False,
            margin=go.layout.Margin(t=30, r=50, b=50, l=60, pad=4),
            showlegend=True,
        )
        ...
        cat_plot_file = "Plots/pub_plots/{}_cat.{}".format(unit.replace(" ", "_"), pformat)
    ...
    jif_plot_file = False
    if any(pd.concat([UnknownJIF, Undersix, sixtonine, ninetotwentyfive, overtwentyfive]).Count):
        fig = go.Figure(
            data=[
                go.Bar(
                    name="JIF unknown",
        ...
        # write plot to a file
        jif_plot_file = "Plots/pub_plots/{}_jif.{}".format(unit.replace(" ", "_"), pformat)
...
def users_plot(unit, year, pformat="png", name_fsize=25, annotation_fsize=34):
    unit_usr_data = affiliate_data[(affiliate_data["Unit"] == unit) & (affiliate_data["Year"] == year)]
    ...
    if any(u_counts):
        ...
        fig = go.Figure(
            go.Pie(
                values=u_counts,
                labels=a_labels,
                text=["{} ({}%)".format(a_labels[i], round(u_counts[i]/sum(u_counts)*100, 1)) for i in range(len(unit_usr_data))],
                hole=0.6,
        ...
        # write plot to a file
        usr_plot_file = "Plots/usr_plots/{}_{}.{}".format(unit.replace(" ", "_"), year, pformat)
        fig.write_image(usr_plot_file)

    return usr_plot_file

publication_plot("Affinity Proteomics Stockholm")
publication_plot("National Genomics Infrastructure")
# users_plot("National Genomics Infrastructure", 2020)
