# Need to modify this script to enable pie charts for multiple years (2022-2023)
...
# also want one per year
# one datafile for each year

NGI_tech_users_2022 = pd.read_excel(
...
NGI_tech_users_2023 = pd.read_excel(
    "Data/NGI Users Technologies 2023.xlsx",
    sheet_name="NGI lists dublets removed",
    header=0,
    engine="openpyxl",
    keep_default_na=False,
...
# want to group by the PI affiliation and different technologies used in each year
NGI_tech_group_22 = (
    NGI_tech_users_2022.groupby(["Technology", "PI affiliation"]).size().reset_index()
)
NGI_tech_group_22.columns = ["Technology", "PI_aff", "Count"]

NGI_tech_group_23 = (
    NGI_tech_users_2023.groupby(["Technology", "PI affiliation"]).size().reset_index()
)
NGI_tech_group_23.columns = ["Technology", "PI_aff", "Count"]

# Add years to data files

NGI_tech_group_22.insert(loc=2, column="Year", value="2022")
NGI_tech_group_23.insert(loc=2, column="Year", value="2023")

# Now abbreviate the affiliations (might need to change/add to this with new data, as more affiliations may appear)
...
abbreviated_tech_data_22 = NGI_tech_group_22.replace(NGI_tech_group_abbr, regex=True)
abbreviated_tech_data_23 = NGI_tech_group_23.replace(NGI_tech_group_abbr, regex=True)
...
def Aff_pies_func(input):
    ...
    if sum(aff_data.Count) > 0:
        ...
        print(
            "Warning: not all Technology year combinations have data - check whether this is expected"
        )


for i in affiliate_data["Year"].unique():
    temp = affiliate_data[(affiliate_data["Year"] == i)]
    for z in temp["Technology"].unique():
        Aff_pies_func(temp[(temp["Technology"] == z)])
...
def Aff_pies_func_stacked_text(input):
    ...
    if not os.path.isdir("Plots/NGI_user_Pies/"):
        os.mkdir("Plots/NGI_user_Pies/")
    if sum(aff_data.Count) > 0:
        fig.write_image(
            "Plots/NGI_user_Pies/{}_{}_affs.svg".format(
                input["Technology"][input["Technology"].first_valid_index()],
                input["Year"][input["Year"].first_valid_index()],
            )
        )
...
for i in affiliate_data["Year"].unique():
    temp = affiliate_data[(affiliate_data["Year"] == i)]
    Aff_pies_func_stacked_text(temp[(temp["Technology"] == "XXX")])
